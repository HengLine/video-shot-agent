...
from videoshot.logger import warning, debug, info, error


class DotEnvLoader:
    """智能 .env 文件加载器"""

    def __init__(self, package_name: str = "videoshot"):
    ...
    def find_dotenv_files(self) -> List[Path]:
        ...
        优先级（从高到低）：
        1. 当前工作目录: ./ .env
        2. 用户配置目录: ~/.config/videoshot/.env
        3. 包安装目录: site-packages/videoshot/.env
        4. 开发模式项目根目录: ../../.env
        """
...
def load_dotenv(override: bool = False) -> Dict[str, str]:
    ...
    使用示例:
        from videoshot.config import load_dotenv
        load_dotenv()  # 自动查找并加载
    """
    global _dotenv_loader
    if _dotenv_loader is None:
        _dotenv_loader = DotEnvLoader("videoshot")
