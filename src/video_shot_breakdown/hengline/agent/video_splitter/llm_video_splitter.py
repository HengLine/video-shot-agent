...
class LLMVideoSplitter(BaseVideoSplitter, BaseAgent):
    ...
    def _get_enhanced_prompt_template(self, context: Dict[str, Any]) -> str:
        ...
        prev_context = ""
        if prev_shot:
            prev_context = f"{prev_shot.description[:100]}... ({prev_shot.duration}s)" if len(prev_shot.description) > 100 else f"{prev_shot.description} ({prev_shot.duration}s)"

        next_context = ""
        if next_shot:
            next_context = f"{next_shot.description[:100]}... ({next_shot.duration}s)" if len(next_shot.description) > 100 else f"{next_shot.description} ({next_shot.duration}s)"
